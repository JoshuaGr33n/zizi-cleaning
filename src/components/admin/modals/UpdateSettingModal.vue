        ...
        <div class="modal-content">
            ...
            <form @submit.prevent="submitForm">
                <div class="form-group">
                    ...
                    <input type="text" id="name" v-model="updatedSetting.name">
                    <p v-if="errors.name" class="error-message">{{ errors.name[0] }}</p>
                </div>

                <div class="form-group">
                    <label for="url">URL:</label>
                    <input type="url" id="url" v-model="updatedSetting.url">
                ...
                <input type="hidden" id="image2" v-model="updatedSetting.image">

                <div class="form-group">
                    <label for="tag">Tag:</label>
                    <input type="text" id="tag" v-model="updatedSetting.tag" required>
                    <p v-if="errors.tag" class="error-message">{{ errors.tag[0] }}</p>
                </div>
                <div class="form-group">
                    <label for="value">Value:</label>
                    <input type="text" id="value" v-model="updatedSetting.value">
                    <p v-if="errors.value" class="error-message">{{ errors.value[0] }}</p>
                </div>
                <div class="form-group">
                    <label for="category">Category:</label>
                    <select id="category" v-model="updatedSetting.category" required>
                        <option>{{ updatedSetting.category }}</option>
                        <option>Phone</option>
                        <option>Email</option>
                ...
                <div class="form-group">
                    ...
                    <textarea id="desc" v-model="updatedSetting.desc"></textarea>
...
export default {
    ...
    data() {
        return {
            updatedSetting: this.setting,
            newSetting: { image: null },
            errors: {},
        };
    },
    methods: {
        submitForm() {
            ...
            const payload = {
                updatedSetting: this.updatedSetting,
                newSetting: {
                    image: this.newSetting.image,
                },
            };
            ...
            this.$store.dispatch('updateSetting', payload)
                .then(() => {
                    this.close();
                    this.showUpdateModal = false; // Close the modal on success
                })
                .catch(errors => {
        ...
        handleFileUpload(event) {
            // this.updatedSetting.image = event.target.files[0];
